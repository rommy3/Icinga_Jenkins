...
if ( $if_status != 1 ) {
	...
	$output = "Average IN: " . $in_traffic . $in_prefix . $suffix . " (" . $in_usage . "%), " . "Average OUT: " . $out_traffic . $out_prefix . $suffix . " (" . $out_usage . "%) ";
...
$output .= "|outUsage=$out_usage%;$warn_usage;$crit_usage inUsage=$in_usage%;$warn_usage;$crit_usage" . " outBandwidth=" . $out_traffic . $out_prefix . $suffix . " inBandwidth=" . $in_traffic . $in_prefix . $suffix . " outAbsolut=$out_traffic_absolut inAbsolut=$in_traffic_absolut";
