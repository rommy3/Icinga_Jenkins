...
#	* 05_13_2016 Changed perf data for absolute Bytes ($in_bytes_abs/$in_bytes_abs) from Bytes units to 'c' for continuous
...
# Set internal timerout to address 'Return code of 25 is out of bounds' errors
eval {
	local %SIG;
	$SIG{ALRM}=
		sub{ 
...
if ( ( $in_ave_pct < $warn_usage ) and ( $out_ave_pct < $warn_usage ) ) {
	$state = 'OK';
	$output = " inBandwidth="
	 . $pin_ave . "B outBandwidth=" . $pout_ave . "B ,";
...
"|"
  ...
  . " Total RX: $in_tot" . "$label, Total TX: $out_tot" . "$label";
