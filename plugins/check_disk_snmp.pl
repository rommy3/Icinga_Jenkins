...
my $argument = "";
my ($WP, $WARNING, $CP, $CRITICAL, $INCLUDE, $EXCLUDE, $HOST, $PASS);
my ($helptext, $skipit, $total, $used, $mount, $templine, $matchline);
my $FINALALERT = 0;
my $ALERT = 0;
my ($fulltext, $text, $avail, $displayavail, $percent, $capnum);
...
my @bulkget = `$SNMPDIR/snmpwalk -v1 -mALL -t 5 -r 3 -c $PASS $HOST hrStorage`;
...
foreach $line (@bulkget) {
  chomp($line);
  print " - $line\n" if ($D > 2);
  if ($line =~ m/hrStorageType\.(\d+) =.*hrStorageFixedDisk$/) {
    $indexid = $1;
...
foreach $indexid (keys(%IDX)) {
  ...
  $avail = int($avail/1000000);
  if ($avail > 1000) {
    # CONVERT TO GB OUTPUT FOR CLARITY
    $displayavail = $avail / 1000;
  ...
  $fulltext .= "[${percent}% full,$displayavail free on $mount] ";
  ...
  if ($CP && $CP eq '%') {
    if ($capnum >= $CRITICAL && $ALERT < 2) {
      ...
      $text .= "${helptext}${red}[${percent}% full,$displayavail free on ${mount}] $font";
    }
  } else {
  ...
  if ($WP && $WP eq '%') {
    if ($capnum >= $WARNING && $ALERT < 1) {
      ...
      $text .= "${helptext}${yellow}[${percent}% full,$displayavail free on ${mount}] $font";
    }
  } else {
    # WARNING LEVEL SET TO MB REMAINING (SPACE FREE)
