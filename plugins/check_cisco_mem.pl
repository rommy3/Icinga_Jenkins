...
$status       = 0;
$returnstring = "";
$opt_f            = undef;      # Output performance data
...
sub main {
 ...
 $temp = sprintf "MEMORY: total: %.2f MB, used: %.2f MB (%.0f%%), free: %.2f MB", ( $memtotal / 1000000 ), ( $mem5minused / 1000000 ), $mempcused, ( $mem5minfree / 1000000 );
        ...
        if (defined($opt_f)) {
                $temp = sprintf 'MEMORY: total: %.2f MB, used: %.2f MB (%.0f%%), free: %.2f MB | Percentage_Used=%3$.2f% MemTotal=%1$.2f, MemUsed=%2$.2f, MemFree=%4$.2f', ( $memtotal / 1000000 ), ( $mem5minused / 1000000 ), $mempcused, ( $mem5minfree / 1000000 );
