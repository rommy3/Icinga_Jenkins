...
=head1 SYNOPSIS
  ...
  ./check_ping.pl --host <host> --loss <warn>,<crit> 
...
printf("PING - Packet loss = %i%%\n", $loss);

exit(2) if($loss>$closs);# Nagios: Critical
exit(1) if($loss>$wloss);# Nagios: Warning
exit(0);# Nagios: OK
